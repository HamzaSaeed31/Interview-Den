...
def parse_resume(args):
    ...
    try:
        ...
        # Extract text and process the resume without saving immediately
        resume_text = resume.extract_text_from_pdf(resume_path)
        job_description = None
        if job_path:
            with open(job_path, 'r', encoding='utf-8') as f:
                job_description = f.read()
        structured_resume = resume.process_resume_with_llm(resume_text, job_description)
...
def parse_job(args):
    ...
    try:
        ...
        # Read and process the job description without saving immediately
        job_description_text = job_parser.read_job_description(job_path)
        structured_job = job_parser.process_job_description_with_llm(job_description_text)
