...
def compare_resume_with_job(resume_data: Dict[str, Any], job_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    # Check cache first - create hash of resume and job data for cache key
    import hashlib
    cache_key_content = json.dumps(resume_data) + json.dumps(job_data)
    ...
    if os.path.exists(cache_file):
        print("Using cached match evaluation result...")
    ...
    system_message = """
    You are a strict resume evaluation assistant. Your task is to compare a candidate's resume against a job description and provide a structured evaluation of the match with high standards.
    
    Compare the provided structured resume and job description data with the following priorities:
    - Technical Skills: Very high weight (must have explicit matches, not inferred skills)
    ...
    4. Be honest and strict in your assessment - this is for screening candidates.
    5. Provide your response as a valid JSON object only, with no additional text.
    """
    ...
    try:
        ...
        try:
            ...
            if json_match:
                try:
                    ...
                    # Cache the result
                    with open(cache_file, 'w', encoding='utf-8') as f:
                        json.dump(parsed_match, f, indent=2, ensure_ascii=False)
        ...
        raise Exception(f"Error in Groq API request: {str(e)}")
    except Exception as e:
        raise Exception(f"Error in LLM matching: {str(e)}")
...
def evaluate_match(resume_file: str, job_file: str, output_file: str = None,
    ...
    """
    Evaluate the match between a resume and a job description
