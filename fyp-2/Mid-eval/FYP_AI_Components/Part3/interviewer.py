...
if not os.getenv("GROQ_API_KEY"):
    FALLBACK_GROQ_KEY = "YOUR_GROQ_API_KEY_HERE" 
    os.environ["GROQ_API_KEY"] = FALLBACK_GROQ_KEY
    print("[INFO] GROQ_API_KEY set from hardcoded fallback key for initialization.")
# ---------------------------------------------------------------------------------
...
class InterviewerBot:
    ...
    def init_session(self) -> str:
        ...
        initial_greeting = f"Hello {self.candidate_name}, thank thank you for joining me today. We have a short time, so let's jump right into the technical discussion. How are you doing?"
        # FIX: Use 'type': 'ai'
    ...
    def _handle_protocol_response(self, bot_text: str) -> str:
        ...
        if bot_text == WARNING_TAG:
            ...
            if self.warning_count > MAX_WARNINGS:
                ...
                self.transcript_history.append({"type": "ai", "content": final_message}) # FIX: use 'type': 'ai'
            ...
            self.transcript_history.append({"type": "ai", "content": warning_message}) # FIX: use 'type': 'ai'
    ...
    def process_turn(self, candidate_reply: str) -> Tuple[str, bool]:
        ...
        if not is_repetition:
            if self.primary_question_count == 0:
                ...
                self.primary_question_count += 1
                self.elaboration_count = 0 
                self.transcript_history.append({"type": "PROTOCOL", "content": PIVOT_TAG}) # FIX: use 'type'
            else:
    ...
    def _initiate_soft_close(self) -> Tuple[str, bool]:
        ...
        self.transcript_history.append({"type": "PROTOCOL", "content": SOFT_CLOSE_TAG}) # FIX: use 'type'
        self.transcript_history.append({"type": "ai", "content": bot_text}) # FIX: use 'type': 'ai'
        self.memory.chat_memory.add_ai_message(AIMessage(content=bot_text))
    ...
    def _continue_soft_close(self, candidate_reply: str) -> Tuple[str, bool]:
        ...
        if SENTINEL_END_PHRASE in bot_text:
            self.is_finished = True
            final_message = bot_text.replace(SENTINEL_END_PHRASE, "").strip()
            self.transcript_history.append({"type": "PROTOCOL", "content": SENTINEL_END_PHRASE}) # FIX: use 'type'
            self.transcript_history.append({"type": "ai", "content": final_message}) # FIX: use 'type': 'ai'
